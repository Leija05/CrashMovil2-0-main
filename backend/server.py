...
# Compatibilidad con distintos nombres de variables para Gemini
if not EMERGENT_LLM_KEY:
    if GOOGLE_API_KEY:
        EMERGENT_LLM_KEY = GOOGLE_API_KEY
    elif GEMINI_API_KEY and not GEMINI_API_KEY.startswith("gemini-"):
        EMERGENT_LLM_KEY = GEMINI_API_KEY

if not GEMINI_MODEL:
    if GEMINI_API_KEY.startswith("gemini-"):
...
async def send_emergency_alerts(user: dict, impact: dict, profile: dict | None, diagnosis: dict | None):
    ...
    if not contacts:
        logger.warning("No emergency contacts to alert")
        return
